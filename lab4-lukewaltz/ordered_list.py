...
def search_helper(self, temp, value):
    if temp == self.dummy:  # traversed whole lst
        return False
    if temp.item > value:  # passed values spot in the list and it wasnt there
        return False
    elif temp.item == value:  # item found
        return True
    else:
...
def reverse_helper(self, temp):
    ...
    if temp.next == self.dummy:
        # end of the ist --> first in rev list
        return [temp.item]
    else:
        # shortens list then appends the new end.
        return reverse_helper(self, temp.next) + [temp.item]
...
class OrderedList:
    ...
    def pop(self, index):
        ...
        if index < 0:
            # negative index
            raise IndexError
        while temp != self.dummy and num_items < index:
            # index not found so next node
            temp = temp.next
            num_items += 1
        if temp == self.dummy:
            ...
            # return val
            return_val = temp.item
            # filling in the whole return val left
